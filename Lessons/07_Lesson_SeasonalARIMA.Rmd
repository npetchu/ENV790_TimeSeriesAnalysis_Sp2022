...
#consider starting with ARMA (1,1,1) by looking at ACF and PACF
```
...
#nsdiff check for seasonal differencing needed, difference on seasonal lag, diff observations by seasonal lag ex. once at lag 12 in this one
...
residential_price_both_diff <- diff(residential_price_trend_diff,lag =12, differences=1) #lag 1 diff first then lag 12 diff next
...
#for lower case p and q, we would choose 1,1 

#for seasonal lags, we have one spike at 12 ACF, multiple spikes for PACF, think this seasonal component is MA since one spike at ACF since negative and 2 spikes at PACF
   #if MA, P=0, Q=1
#one spike at ACF, previous observations at seasonal lag important at first year, what happened before that doesn't matter, so we have short memory, just one spike at ACF and few spikes at PACF

#if long memory, we have AR model

#compare AIC for seasonality removed, AIC for dataset that has seasonality there, can't compare AIC since not based on same initial variable, not based on same initial variable since one is ARIMA (not handle seasonality) and one is SARIMA (handle seasonality), not fair to compare ARIMA errors before you put back seasonality, always need to compare consider same base dataset

#goal is to manipulate PDQ to bring ACF/PACF within error bars and then decrease AIC
...
print(SARIMA_manual) 
#just referring to original column, identify original order before, order with respect to non-seasonal part
#for seasonal, order is 011
#do drift= false because we are differencing twice, so more likely mean is now zero, so don't want to include any drift
...
Easier to remove seasonal component since have 3 parameters to estimate, but is this the best way? No since SARIMA allows for more variability for the seasonal trend. Allow for more possibility for seasonal component. Non seasonal ARIMA, assume seasonal component is constant over time. Could be that it is and SARIMA and ARIMA lead to similar results but if we can model it and we know how to identify other 3 parameters is the best way to do it. 

auto.arima will give the lowest AIC and we are looking at plots to estimate the order of the model.. but situations where auto.arima would not converge and find best model.. reasons this could happen by increasing ARMA, not changing AIC too much so never leave the loop.. important to know about model estimation so can reduce possibilities of auto.arima function
